...
use super::controller::ControllerIdentity;
...
impl PairSetup {
    ...
    pub fn process_m6(&mut self, response: &[u8]) -> Result<SharedSecret> {
        ...
        crate::airplay::crypto::ed25519::verify(&server_pk_arr, &message, &server_sig_arr).map_err(|_| {
            self.state = PairSetupState::Failed;
            Error::Pairing(PairingError::Protocol(
...
struct MockSrpServer {
    params: crate::airplay::crypto::srp::SrpParams,
    salt: [u8; 16],
...
impl MockSrpServer {
    fn new(identity: &[u8], password: &[u8], salt: [u8; 16]) -> Self {
        ...
        let params = crate::airplay::crypto::srp::SrpParams::default();
...
mod tests {
    ...
    /// Tests for the transient pairing implementation using SRP.
    ///
