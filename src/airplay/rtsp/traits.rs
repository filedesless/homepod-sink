...
use super::{RtspRequest, RtspResponse};
...
mod tests {
    ...
    async fn mock_transport_returns_configured_response() {
        ...
        // Call the mock
        let request = RtspRequest::new(crate::airplay::rtsp::RtspMethod::Get, "/info");
    ...
    async fn mock_transport_can_simulate_error() {
        ...
        mock.expect_send().returning(|_request| {
            Box::pin(async {
                Err(crate::airplay::core::error::RtspError::ConnectionRefused.into())
            })
        });
