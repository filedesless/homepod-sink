...
impl StreamConfig {
    ...
    /// Create config for realtime streaming (ALAC, NTP timing).
    ///
    /// This only sets stream parameters (codec, timing protocol, latency) —
    /// it does NOT select AirPlay 1 (RAOP) vs AirPlay 2. The actual
    /// pairing/RTSP protocol used depends entirely on which connection type
    /// you construct: `RaopConnection`/`AirPlayClient` via RAOP methods for
    /// AirPlay 1, or `Connection`/`AirPlayClient::connect()` for AirPlay 2
    /// (HomeKit pairing, ChaCha20-Poly1305 encryption). This config works
    /// with either — it was previously named `airplay1_realtime()`, which
    /// wrongly implied it forced the AirPlay 1 protocol.
    pub fn realtime_ntp() -> Self {
        Self::default()
    }
...
mod tests {
    use super::*;
    use crate::airplay::core::codec::{AudioCodec, SampleRate};
