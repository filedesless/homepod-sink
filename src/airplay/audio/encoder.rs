...
///
/// Only ALAC is implemented — this sender always uses
/// `StreamConfig::realtime_ntp()`, whose default `AudioFormat` is ALAC, and
/// never negotiates AAC (buffered-mode streaming, which AAC is for, isn't
/// used here).
pub fn create_encoder(format: AudioFormat) -> Result<Box<dyn AudioEncoder>> {
    match format.codec {
        AudioCodec::Alac => Ok(Box::new(AlacEncoder::new(format)?)),
        _ => Err(crate::airplay::core::error::StreamingError::InvalidFormat(
...
mod tests {
    ...
    mod audio_validation {
        ...
        fn encode_real_audio_file() {
            use std::env;
            use crate::airplay::audio::decoder::AudioDecoder;
