...
use crate::airplay::core::{AudioFormat, error::Result};
use std::fs::File;
use std::io::Cursor;
use std::path::Path;
...
pub struct AudioDecoder {
    ...
    resampler: Option<super::resampler::Resampler>,
}
...
impl AudioDecoder {
    ...
    pub fn open(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let file = File::open(path).map_err(|e| {
            crate::airplay::core::error::StreamingError::InvalidFormat(format!(
                "Failed to open file: {}",
                e
            ))
        })?;
    ...
    fn from_media_source(mss: MediaSourceStream, hint: Hint) -> Result<Self> {
        ...
        let track = format
            ...
            .ok_or_else(|| {
                crate::airplay::core::error::StreamingError::InvalidFormat(
    ...
    pub fn seek(&mut self, position_samples: u64) -> Result<()> {
        ...
        self.format
            ...
            .map_err(|e| {
                crate::airplay::core::error::StreamingError::Encoding(format!("Seek failed: {}", e))
            })?;
    ...
    pub fn decode_frame(&mut self) -> Result<Option<DecodedFrame>> {
        ...
        loop {
            let packet = match self.format.next_packet() {
                ...
                Err(e) => {
                    return Err(crate::airplay::core::error::StreamingError::Encoding(format!(
    ...
    pub fn decode_resampled(
        ...
        if source_rate != target_rate && self.resampler.is_none() {
            self.resampler = Some(super::resampler::Resampler::new(
                source_rate,
                target_rate,
                self.channels,
            )?);
    ...
    mod resampling {
        ...
        fn resample_44100_to_48000() {
            ...
            let target = AudioFormat {
                codec: crate::airplay::core::AudioCodec::Alac,
