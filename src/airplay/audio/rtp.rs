    ...
    pub fn parse(data: &[u8]) -> Result<Self> {
        if data.len() < 12 {
            return Err(Error::Parse(
                crate::airplay::core::error::ParseError::InvalidFormat("RTP header too short".into())
            ));
        }
...
impl RtpPacket {
    ...
    pub fn parse(data: &[u8]) -> Result<Self> {
        if data.len() < 12 {
            return Err(Error::Parse(
                crate::airplay::core::error::ParseError::InvalidFormat("RTP packet too short".into())
            ));
        }
    ...
    pub fn parse_encrypted(data: &[u8]) -> Result<Self> {
        ...
        if data.len() < 36 {
            return Err(Error::Parse(
                crate::airplay::core::error::ParseError::InvalidFormat("Encrypted RTP packet too short".into())
            ));
        }
    ...
    pub fn send(&mut self, packet: &RtpPacket) -> Result<()> {
        let socket = self.socket.as_ref()
            .ok_or_else(|| Error::Streaming(
                crate::airplay::core::error::StreamingError::Encoding("Socket not bound".into())
            ))?;
