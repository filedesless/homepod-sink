...
use crate::airplay::core::{StreamConfig, error::Result};
...
/// Jitter/timing stats shared between the real-time sender thread and a
/// separate, non-realtime task that logs them.
///
/// The sender thread must never do anything with unpredictable latency
/// (allocation, locks, syscalls) between its precise-timing wait and the
/// actual `send_to()` calls - `tracing::info!`/`warn!` do all of those
/// internally (subscriber lock, string formatting, a write syscall), and
/// placing them directly in that path caused audible crackling even with
/// stdout redirected to /dev/null (so the cost is the call itself, not
/// where its output goes). This struct lets the sender thread record stats
/// with plain atomic stores (no formatting, no I/O) and hand off the actual
/// logging to a separate low-priority task.
#[derive(Default)]
struct SenderStats {
    packet_count: AtomicU64,
    bursts: AtomicU64,
    max_jitter_ms_bits: AtomicU64,
    jitter_sum_ms_bits: AtomicU64,
    jitter_exceed_count: AtomicU64,
    /// Incremented each time stats are updated, so the logger task can tell
    /// whether there's anything new to report since it last looked.
    stats_version: AtomicU64,
    last_logged_stats_version: AtomicU64,
    /// Bumped on each individual high-jitter burst (for the WARN-level log),
    /// independent of the periodic stats snapshot above.
    jitter_warn_version: AtomicU64,
    last_logged_jitter_warn_version: AtomicU64,
    jitter_warn_burst_num: AtomicU64,
    jitter_warn_interval_ms_bits: AtomicU64,
    jitter_warn_target_ms_bits: AtomicU64,
    jitter_warn_jitter_ms_bits: AtomicU64,
    /// Bumped each time the sender thread falls more than one burst behind.
    fell_behind_version: AtomicU64,
    last_logged_fell_behind_version: AtomicU64,
    fell_behind_ms_bits: AtomicU64,
}

impl SenderStats {
    fn record_stats(&self, packet_count: u64, bursts: u64, max_jitter_ms: f64, jitter_exceed_count: u64) {
        self.packet_count.store(packet_count, Ordering::Relaxed);
        self.bursts.store(bursts, Ordering::Relaxed);
        self.max_jitter_ms_bits.store(max_jitter_ms.to_bits(), Ordering::Relaxed);
        self.jitter_exceed_count.store(jitter_exceed_count, Ordering::Relaxed);
        self.stats_version.fetch_add(1, Ordering::Relaxed);
    }

    fn record_jitter_sum(&self, jitter_sum_ms: f64) {
        self.jitter_sum_ms_bits.store(jitter_sum_ms.to_bits(), Ordering::Relaxed);
    }

    fn record_jitter_warn(&self, burst_num: u64, interval_ms: f64, target_ms: f64, jitter_ms: f64) {
        self.jitter_warn_burst_num.store(burst_num, Ordering::Relaxed);
        self.jitter_warn_interval_ms_bits.store(interval_ms.to_bits(), Ordering::Relaxed);
        self.jitter_warn_target_ms_bits.store(target_ms.to_bits(), Ordering::Relaxed);
        self.jitter_warn_jitter_ms_bits.store(jitter_ms.to_bits(), Ordering::Relaxed);
        self.jitter_warn_version.fetch_add(1, Ordering::Relaxed);
    }

    fn record_fell_behind(&self, behind_ms: f64) {
        self.fell_behind_ms_bits.store(behind_ms.to_bits(), Ordering::Relaxed);
        self.fell_behind_version.fetch_add(1, Ordering::Relaxed);
    }

    /// Poll for new stats/warnings and log anything not yet logged. Meant to
    /// be called periodically from a non-realtime task.
    fn log_pending(&self) {
        let version = self.stats_version.load(Ordering::Relaxed);
        if version != self.last_logged_stats_version.load(Ordering::Relaxed) {
            self.last_logged_stats_version.store(version, Ordering::Relaxed);
            let packet_count = self.packet_count.load(Ordering::Relaxed);
            let bursts = self.bursts.load(Ordering::Relaxed);
            let max_jitter_ms = f64::from_bits(self.max_jitter_ms_bits.load(Ordering::Relaxed));
            let avg_jitter = f64::from_bits(self.jitter_sum_ms_bits.load(Ordering::Relaxed))
                / (bursts.saturating_sub(1)).max(1) as f64;
            let jitter_exceed_count = self.jitter_exceed_count.load(Ordering::Relaxed);
            tracing::info!(
        ...
        let jversion = self.jitter_warn_version.load(Ordering::Relaxed);
        if jversion != self.last_logged_jitter_warn_version.load(Ordering::Relaxed) {
            self.last_logged_jitter_warn_version.store(jversion, Ordering::Relaxed);
            tracing::warn!(
                "JITTER burst#{}: interval={:.3}ms target={:.3}ms jitter={:+.3}ms",
                self.jitter_warn_burst_num.load(Ordering::Relaxed),
                f64::from_bits(self.jitter_warn_interval_ms_bits.load(Ordering::Relaxed)),
                f64::from_bits(self.jitter_warn_target_ms_bits.load(Ordering::Relaxed)),
                f64::from_bits(self.jitter_warn_jitter_ms_bits.load(Ordering::Relaxed)),
            );
        }

        let fversion = self.fell_behind_version.load(Ordering::Relaxed);
        if fversion != self.last_logged_fell_behind_version.load(Ordering::Relaxed) {
            self.last_logged_fell_behind_version.store(fversion, Ordering::Relaxed);
            tracing::warn!(
                "Sender thread fell behind by {:.2}ms, resetting deadline",
                f64::from_bits(self.fell_behind_ms_bits.load(Ordering::Relaxed))
            );
        }
    }
}
    ...
    loop {
        ...
        match msg {
            ...
            SenderMessage::Packet { wire_packets, sync_data } => {
                ...
                // Track jitter stats (per burst). Recording is plain atomic
                // stores only (no formatting, no I/O) - actual logging
                // happens later on a separate, non-realtime task via
                // SenderStats::log_pending(). This thread is SCHED_FIFO
                // real-time and solely responsible for precisely timed
                // packet transmission; tracing::info!/warn! synchronously
                // format and write on whatever thread calls them, and doing
                // that directly here was audible as crackling even with
                // stdout redirected to /dev/null (so the cost is the calls
                // themselves, not where their output goes).
                ...
                if packet_count > burst_size as u64 {
                    ...
                    // Snapshot stats every 500 packets (~4s)
                    if packet_count % 500 < burst_size as u64 {
                        let bursts = packet_count / burst_size as u64;
                        stats.record_jitter_sum(jitter_sum_ms);
                        stats.record_stats(packet_count, bursts, max_jitter_ms, jitter_exceed_count);
                    }
                }
                ...
                #[cfg(target_os = "linux")]
                {
                    ...
                    if now_ns > next_deadline_ns + burst_duration_ns {
                        stats.record_fell_behind((now_ns - next_deadline_ns) as f64 / 1_000_000.0);
                ...
                #[cfg(not(target_os = "linux"))]
                {
                    ...
                    if now > next_deadline + frame_duration * burst_size as u32 {
                        stats.record_fell_behind((now - next_deadline).as_secs_f64() * 1000.0);
                        next_deadline = now;
                    }
                }
            }
...
pub struct AudioStreamer {
    ...
    /// Jitter/timing stats recorded by the real-time sender thread and
    /// logged by a separate, non-realtime task (see `SenderStats` docs).
    sender_stats: Arc<SenderStats>,
    /// Task that periodically logs `sender_stats`.
    stats_logger_task: Option<JoinHandle<()>>,
}
...
impl Clone for AudioStreamer {
    fn clone(&self) -> Self {
        Self {
            ...
            sender_stats: Arc::clone(&self.sender_stats),
            stats_logger_task: None, // Can't clone JoinHandle
        }
    }
}
...
impl AudioStreamer {
    ...
    pub async fn handle_retransmit(&self, request: &super::rtp::RetransmitRequest) -> crate::airplay::core::error::Result<u16> {
    ...
    pub async fn handle_retransmit_for_target(&self, index: usize, request: &super::rtp::RetransmitRequest) -> crate::airplay::core::error::Result<u16> {
        let inner = self.inner.lock().await;
        ...
        if self.task.is_none() {
...

            {
                ...
                if !targets.is_empty() {
                    ...
                    let stats = Arc::clone(&self.sender_stats);
                    self.stats_logger_task = Some(tokio::spawn(async move {
                        loop {
    ...
    pub async fn stop(&mut self) -> Result<()> {
        ...
        // Cancel the stats logger task
        if let Some(task) = self.stats_logger_task.take() {
            task.abort();
        }
...
fn decode_some_inner(inner: &mut StreamerInner) -> Result<()> {
    ...
    for _ in 0..3 {
        ...
        if let Some(frame) = frame {
            let audio_frame = super::AudioFrame::new(frame.samples, frame.timestamp);
            inner
                .buffer
                .push(audio_frame)
                .map_err(|_| crate::airplay::core::error::StreamingError::BufferOverflow)?;
        } else if is_live {
...
async fn run_streamer(
    ...
    loop {
            ...
            if let Some(frame) = frame {
                ...
                if spatial_enabled {
                    ...
                    let encoder = guard
                        .encoder
                        .as_mut()
                        .ok_or_else(|| crate::airplay::core::error::StreamingError::Encoding("Encoder missing".into()))?;
                ...
                if !guard.rtp_senders.is_empty() {
                    if let Some(ref tx) = sender_tx {
                        ...
                        drop(guard);
                        let mut msg = SenderMessage::Packet { wire_packets, sync_data };
                        // crossbeam_channel::Sender has no async API, so a plain
                        // `tx.send()` here would block this tokio worker thread for
                        // as long as the channel (capacity 8) is full — which happens
                        // routinely, since the sender thread drains at real-time pace.
                        // That starves every other task scheduled on the same worker
                        // (RTSP keepalives, control-port receiver, other streams in a
                        // group). spawn_blocking avoided that but paid its own
                        // per-packet scheduling overhead. Instead, poll with
                        // try_send and yield to the runtime between attempts so this
                        // task never blocks the OS thread.
                        loop {
                        ...
                        // The try_send loop above only throttles this producer when
                        // the channel is already full — otherwise (the common case,
                        // since encoding one packet takes ~0.02-0.04ms vs. the
                        // sender thread draining one packet roughly every
                        // frame_duration) it returns immediately and control falls
                        // through to the pacing sleep below instead of `continue`ing
                        // straight back to the top of the loop. Skipping that pacing
                        // here used to spin this tokio worker at ~100% CPU refilling
                        // the 8-slot channel far faster than real-time playback ever
                        // drains it.
                    } else {
        ...
        // Deadline-based pacing applies whether or not a sender thread is in
        // use. The channel's backpressure (try_send/yield_now above) only
        // throttles this producer once the channel is already full; the rest
        // of the time — the common case, since encoding one packet is far
        // cheaper than frame_duration — it fell through to a bare
        // `yield_now().await`, which just re-queues the task instead of
        // actually waiting, so this loop free-ran as fast as the CPU allowed
        // and pegged a core encoding/enqueuing packets the sender thread
        // wasn't ready to drain yet.
        next_deadline += frame_duration;
        let now = Instant::now();
        if next_deadline > now {
            tokio::time::sleep(next_deadline - now).await;
        } else if has_sender_thread {
            // Fell behind: don't sleep past a deadline that's already gone,
            // but still yield so this doesn't starve other tasks on the
            // worker while catching up.
            tokio::task::yield_now().await;
        }
    }
...
mod tests {
    use super::*;
    use crate::airplay::core::StreamConfig;
