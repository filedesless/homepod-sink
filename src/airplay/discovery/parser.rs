...
use crate::airplay::core::error::ParseError;
use crate::airplay::core::{Device, DeviceId, Features, Version};
...
impl TxtRecordParser {
    ...
    pub fn parse_airplay_txt(
        ...
        // Parse group UUID. Some devices (e.g. HomePods that are part of a
        // stereo pair or multi-room group) advertise a compound value here:
        // "<own-group-uuid>+<parent-group-uuid>", not a single UUID. Parse
        // just the first segment and don't fail the whole device parse over
        // this field — a HomePod's actual streamability doesn't depend on
        // us understanding its exact group topology.
        let group_id = txt
            .get("gid")
            .and_then(|gid| gid.split('+').next())
            .and_then(|gid| uuid::Uuid::parse_str(gid).ok());
        ...
        // Parse parent group UUID (see `group_id` above re: compound values).
        let parent_group_id = txt
            .get("pgid")
            .and_then(|pgid| pgid.split('+').next())
            .and_then(|pgid| uuid::Uuid::parse_str(pgid).ok());
        ...
        // Parse tight sync UUID (see `group_id` above re: compound values).
        let tight_sync_id = txt
            .get("tsid")
            .and_then(|tsid| tsid.split('+').next())
            .and_then(|tsid| uuid::Uuid::parse_str(tsid).ok());
...
mod tests {
    ...
    mod parse_airplay_txt {
        ...
        #[test]
        fn compound_gid_does_not_fail_whole_parse() {
            // Real HomePods that are part of a stereo pair / multi-room
            // group advertise `gid` as "<own-uuid>+<parent-uuid>", not a
            // single UUID. Previously this made the *entire* device parse
            // fail (via `?`), silently dropping the AirPlay2 advertisement
            // and leaving discovery to fall back to the far more limited
            // RAOP-only record for the device.
            let txt = make_txt(&[
                ("deviceid", "5A:00:A7:18:FA:F7"),
                ("model", "AudioAccessory1,1"),
                (
                    "gid",
                    "FA8FB3DE-B9F1-4970-B8D7-7B9B56DCDD6D+C6CAA070-3C7D-42F4-A6CE-E235DF7C5D69",
                ),
            ]);
            ...
            assert_eq!(device.model, "AudioAccessory1,1");
            assert_eq!(
                device.group_id,
                Some(uuid::Uuid::parse_str("FA8FB3DE-B9F1-4970-B8D7-7B9B56DCDD6D").unwrap())
            );
        }
