...
use crate::airplay::core::{Device, DeviceId, error::Result};
use uuid::Uuid;
...
impl DeviceGroup {
    ...
    pub fn remove_member(&mut self, id: &DeviceId) -> Result<()> {
        ...
        if id == &self.leader_id {
            return Err(crate::airplay::core::error::RtspError::SetupFailed(
    ...
    pub fn set_member_volume(&mut self, id: &DeviceId, volume: f32) -> Result<()> {
        if let Some(member) = self.members.get_mut(id) {
            ...
            Err(crate::airplay::core::error::DiscoveryError::DeviceNotFound(
                id.to_mac_string()
            ).into())
        }
    }
...
mod tests {
    ...
    use crate::airplay::core::device::Version;
