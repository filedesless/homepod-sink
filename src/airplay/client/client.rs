...
use crate::airplay::core::error::{Error, RtspError, DiscoveryError};
use crate::airplay::discovery::{ServiceBrowser, Discovery};
use crate::airplay::audio::{AudioDecoder, AlacEncoder, AudioStreamer, LiveAudioDecoder, LiveFrameSender, EqConfig, EqParams, RetransmitRequest, SpatialParams, SpeakerConfig};
use std::sync::Arc;
use std::sync::atomic::Ordering;
use std::path::Path;
use std::net::UdpSocket;
use std::time::Duration;
use super::{Connection, DeviceGroup, PlaybackState, EventHandler, ClientEvent};
...
impl AirPlayClient {
    ...
    pub async fn connect_group(&mut self, devices: &[Device]) -> Result<()> {
        ...
        config.timing_protocol = crate::airplay::core::stream::TimingProtocol::Ptp;
        config.ptp_mode = crate::airplay::core::PtpMode::Master;

        // Generate ALAC magic cookie if needed
        if config.audio_format.codec == crate::airplay::core::AudioCodec::Alac && config.asc.is_none() {
            let temp_encoder = AlacEncoder::new(config.audio_format.clone())
                .map_err(|e| Error::Streaming(crate::airplay::core::error::StreamingError::Encoding(
    ...
    pub async fn play_file_to_group(&mut self, path: impl AsRef<Path>) -> Result<()> {
        ...
        self.stream_stats = super::stats::StreamStats::with_device_count(device_count);
